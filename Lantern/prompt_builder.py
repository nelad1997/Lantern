...
def build_prompt(action: ActionType, focus: str, instructions: str = "") -> str:
    ...
    if action == ActionType.SEGMENT:
        return (
            ...
            "--- SEGMENTATION RULES ---\n"
            "1. SKIP MAIN TITLE: If the text starts with a main document title or a stand-alone headline, SKIP IT. Do NOT include it as a block. The first block should be the first actual paragraph of content.\n"
            "2. LOGICAL ARGUMENT UNITS: Every distinct argumentative point, sub-header, or list item (e.g., 'Artificial Stress:', 'The Pace Problem:') should be its own block. The user expects all ~16 logical units in the provided example to be represented.\n"
            "3. NO MERGING: Do not merge a header with the paragraph below it if they are distinct in the user's draft. Treat them as separate argument units.\n"
            ...
            "--- OUTPUT FORMAT (STRICT) ---\n"
            "Respond ONLY with a numbered list of argumentative blocks in this exact format:\n\n"
            "Block 1:\n"
